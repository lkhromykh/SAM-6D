...
def get_parser():
    ...
    parser.add_argument("--det_score_thresh", default=0.2, help="The score threshold of detection") #default 0.2 !
...
def _get_template(path, cfg, tem_index=1):
    ...
    xyz = np.load(xyz_path).astype(np.float32) / 1000 # default = 1000.0  !!!
...
def get_test_data(rgb_path, depth_path, cam_path, cad_path, seg_path, det_score_thresh, cfg):
    ...
    model_points = mesh.sample(cfg.n_sample_model_point).astype(np.float32) / 1000 # default = 1000 !!!
    ...
    if all_cloud:
        ...
        print("The list all_cloud is empty")

    if all_rgb:
        ret_dict['rgb'] = torch.stack(all_rgb).cuda()
    else:
        print("The list all_rgb is empty")

    if all_rgb_choose:
        ret_dict['rgb_choose'] = torch.stack(all_rgb_choose).cuda()
    else:
        print("The list all_rgb_choose is empty")

    if all_score:
        ret_dict['score'] = torch.FloatTensor(all_score).cuda()
    else:
        print("The list all_score is empty")
...
if __name__ == "__main__":
    ...
    print()
    print(f"PEM model loading time: {round(execution_time, 2)} seconds\n")
    print()
    ...
    print()
    print(f"Extracting templates and loading input data: {round(execution_time, 2)} seconds\n")
    print()
    ...
    execution_time = end_time - start_time
    print(f"6D Pose Estimation Time: {round(execution_time, 2)} seconds\n")
